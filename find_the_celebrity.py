...
class Solution:
    def findCelebrity_BruteForce(self, N: int) -> int:
        # Brute force N^2
        for src in range(N): # celebrity candidate
            #print(f'celeb candiadate {src}')
            early_break = False
            for dst in range(N):
                #print(f'dst {dst}')
                if src == dst:
                    ...
                    if knows(src, dst) == True or knows(dst, src) == False:
                        # This cannot be the celebrity
                        #print(f'Early break {knows(src, dst)} {knows(dst, src)}')
                        early_break = True
                        break
